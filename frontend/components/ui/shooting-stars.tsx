  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      if (time >= nextStarTimeRef.current) {
        starsRef.current = [
          ...starsRef.current,
          getStarProps(width, height),
        ];
        nextStarTimeRef.current =
          time + Math.random() * (maxDelay - minDelay) + minDelay;
      }
      ...
      starsRef.current.forEach((star) => {
        ...
        line.setAttribute(
          "stroke",
          `url(#gradient-${Math.floor(star.id)})`
        );
        line.setAttribute("stroke-width", String(starHeight));
        line.setAttribute("stroke-linecap", "round");
        line.setAttribute(
          "transform",
          `rotate(${star.angle}, ${star.x}, ${star.y})`
        );
