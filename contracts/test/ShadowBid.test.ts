...
async function deployFullFixture() {
  ...
  const buyerECIESPubKey =
    "04abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef12345678";
...
describe("Scenario 1: Happy Path", function () {
  ...
  it("1.2 — Vault initial state is correct", async function () {
    const { vault, closeTime, depositRequired } =
  ...
  it("1.4 — Supplier A commits on-chain", async function () {
    const { vault, supplierA, depositRequired } =
      await loadFixture(deployFullFixture);
    const price = 100000000n;
    const salt = randomSalt();
    const commitHash = computeCommitHash(price, salt, supplierA.address);
    ...
    await expect(
      vault
        .connect(supplierA)
        .commitBid(commitHash, "root_hash_A", { value: depositRequired }),
    )
  ...
  it("1.5-1.6 — All 3 suppliers commit different hashes", async function () {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await vault
        .connect(suppliers[i])
        .commitBid(hash, `root_${i}`, { value: depositRequired });
    }
  ...
  it("1.7 — Bid count is 3", async function () {
    ...
    for (const s of suppliers) {
      ...
      await vault
        .connect(s)
        .commitBid(hash, "root", { value: depositRequired });
    }
  ...
  it("1.15-1.16 — All deposits refunded after settlement", async function () {
    ...
    const balBefore = await Promise.all(
      suppliers.map((s) => ethers.provider.getBalance(s.address)),
    );
  ...
  it("1.17 — Audit data accessible by auditor", async function () {
    ...
    const [_suppliers, _hashes, _roots, _prices, _revealed] = await vault
      .connect(auditor)
      .getAuditData();
...
describe("Scenario 2: Penalty — Non-Reveal Slashing", function () {
  it("2.1-2.3 — Only Supplier A reveals, B does not", async function () {
    ...
    await vault
      .connect(supplierA)
      .commitBid(hashA, "rootA", { value: depositRequired });
    await vault
      .connect(supplierB)
      .commitBid(hashB, "rootB", { value: depositRequired });
  ...
  it("2.4-2.5 — Settlement slashes non-revealer, winner is A", async function () {
    ...
    await expect(tx)
      .to.emit(vault, "DepositSlashed")
      .withArgs(supplierB.address, depositRequired, buyer.address);
    await expect(tx)
      .to.emit(vault, "BidDisqualified")
  ...
  it("2.6 — Buyer received slashed deposit", async function () {
    ...
    await vault
      .connect(supplierA)
      .commitBid(
        computeCommitHash(100000000n, saltA, supplierA.address),
        "rootA",
    ...
    await vault
      .connect(supplierB)
      .commitBid(
        computeCommitHash(95000000n, saltB, supplierB.address),
        "rootB",
        { value: depositRequired },
      );
...
describe("Scenario 3: Security — Cheat Attempts", function () {
  it("3.1 — Reveal with wrong salt reverts", async function () {
    const { vault, supplierA, depositRequired, closeTime } =
      await loadFixture(deployFullFixture);
  ...
  it("3.4 — Commit by non-whitelisted address reverts", async function () {
    const { vault, outsider, depositRequired } =
      await loadFixture(deployFullFixture);
  ...
  it("3.5 — Double commit by same supplier reverts", async function () {
    ...
    await expect(
      vault
        .connect(supplierA)
        .commitBid(hash, "root2", { value: depositRequired }),
  ...
  it("3.6 — Settle before reveal deadline reverts", async function () {
    ...
    await vault
      .connect(supplierA)
      .commitBid(
        computeCommitHash(100000000n, salt, supplierA.address),
        "root",
        { value: depositRequired },
      );
    ...
    await expect(vault.connect(buyer).settle()).to.be.revertedWith(
  ...
  it("3.7 — Settle by non-buyer reverts", async function () {
    ...
    await expect(vault.connect(supplierA).settle()).to.be.revertedWith(
  ...
  it("3.8 — Audit by non-auditor reverts", async function () {
    ...
    await expect(vault.connect(outsider).getAuditData()).to.be.reverted; // AccessControl revert
  });
  ...
  it("3.9 — Cancel after OPEN phase reverts", async function () {
    ...
    await vault
      .connect(supplierA)
      .commitBid(
        computeCommitHash(100000000n, randomSalt(), supplierA.address),
        "root",
        { value: depositRequired },
      );
    await time.increaseTo(closeTime);
    ...
    await expect(vault.connect(buyer).cancel()).to.be.revertedWith(
      "Can only cancel in OPEN phase",
    );
  });
  ...
  it("3.10 — Commit with zero hash reverts", async function () {
    ...
    await expect(
      vault
        .connect(supplierA)
        .commitBid(ethers.ZeroHash, "root", { value: depositRequired }),
...
describe("Scenario 7: Cancellation", function () {
  it("7.1 — Cancel with deposits refunds all", async function () {
    ...
    await vault
      .connect(supplierB)
      .commitBid(
        computeCommitHash(95000000n, randomSalt(), supplierB.address),
        "rootB",
        { value: depositRequired },
      );
  ...
  it("7.2 — Deposits refunded on cancel", async function () {
    ...
    await expect(tx)
      .to.emit(vault, "DepositReturned")
      .withArgs(supplierB.address, depositRequired);
  });
  ...
  it("7.4 — Cancel by non-buyer reverts", async function () {
    const { vault, supplierA } = await loadFixture(deployFullFixture);
    await expect(vault.connect(supplierA).cancel()).to.be.revertedWith(
...
describe("Scenario 8: Factory & Access Control", function () {
  ...
  it("8.2 — Grant BUYER_ROLE enables vault creation", async function () {
    const { factory, owner, buyer, addr1, auditor } =
      await loadFixture(deployFactoryOnly);
    await factory.grantBuyerRole(buyer.address);
    const now = await time.latest();
  ...
  it("8.4 — getAllVaults returns correct list", async function () {
    const { factory, owner, addr1, auditor } =
      await loadFixture(deployFactoryOnly);
    const now = await time.latest();
